...
export class Player {
    ...
    async movePaddle(direction: number, socket1: any, socket2: any, screen: Screen) {
        ...
        while (this.paddle.movingPaddle)
        {
            ...
            if (socket1 !== null)
                socket1.send(JSON.stringify({type: 'paddlePos', paddle: this.paddle.getPaddleData(), paddlePlayer: this.playerGameId, screen: screen}));
            if (socket2 !== null)
                socket2.send(JSON.stringify({type: 'paddlePos', paddle: this.paddle.getPaddleData(), paddlePlayer: this.playerGameId, screen: screen}));
            await wait(16);
        }
...
export class Game {
    ...
    async gameInit(player1Id: number, player2Id: number, socket1: any, socket2: any) {
        ...
        this.player1.setPlayerGameId(1);
        this.player2.setPlayerGameId(2);
    ...
    async stopPaddle(playerId: number) {
        if (playerId === this.player1.getPlayerGameId())
            await this.player1.stopPaddle();
        else if (playerId === this.player2.getPlayerGameId())
            await this.player2.stopPaddle();
    }

    async getPlayerId(playerId: number): Promise<Player> {
        if (playerId === this.player1.getPlayerGameId())
            return (this.player1);
        else if (playerId === this.player2.getPlayerGameId())
            return (this.player2);
